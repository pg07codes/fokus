...
export default function TaskInput() {
  ...
  const [generateFeedbackForTask, setGenerateFeedbackForTask] =
    useState(undefined);
  const [generateFeedbackForTime, setGenerateFeedbackForTime] =
    useState(undefined);
    ...
    if (e.key === "Enter" && task.trim().length >= 1 && isInputValid) {
      ...
      if (temp.length !== 1) {
        if (!isNaN(parseInt(temp[temp.length - 1]))) {
          ...
          "wpfmeWPFME".includes(temp[temp.length - 1][1])
        ) {
      ...
      if (meta.focussedTaskIndex !== -1)
        dispatch(focusOnTask(meta.focussedTaskIndex + 1));
      dispatch(create(newTask));
      dispatch(incrementGlobalKey());
      if (label !== null)
        dispatch(updateLabelCount({ oldLabel: null, newLabel: label }));
      setTask("");
      setTime(defaultTime);
